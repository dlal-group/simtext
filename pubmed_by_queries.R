  ...
  print(data)
    ...
    if (as.numeric(pubmed_search$Count) == 0) {
      ...
      return(data)
            ...
            if (length(PMIDs) > 0) {
              data[row, sapply(seq(length(PMIDs)), function(i) {paste0("PMID_", i) })] <- PMIDs
          ...
          while (is.null(out.data)) {
              ...
              out.data <- tryCatch({
                tmpConnect <- suppressWarnings(url(efetch_url,
                                                   open = "rb",
                                                   encoding = "UTF8"))
                suppressWarnings(readLines(tmpConnect,
                                           warn = FALSE,
                                           encoding = "UTF8"))
              }, error = function(e) {
                print(e)
              }, finally = {
                try(suppressWarnings(close(tmpConnect)),
                    silent = TRUE)
              })
              ...
              if (!is.null(out.data) &&
                  ...
                  grepl("<ERROR>", substr(paste(utils::head(out.data, n = 100),
                                                collapse = ""), 1, 250))) {
                  out.data <- NULL
              }
              try_num <- try_num + 1
          ...
          if (length(abstracts) > 0) {
            data[row, sapply(seq(length(abstracts)),
                             function(i){paste0("ABSTRACT_", i)} )] <- abstracts
